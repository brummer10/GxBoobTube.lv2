...
static void resize_event(gx_boobtubeUI *ui);
...
cairo_surface_t *cairo_image_surface_create_from_stream (gx_boobtubeUI* ui, const unsigned char* name) {
...
static LV2UI_Handle instantiate(const struct _LV2UI_Descriptor * descriptor,
	...
	gx_boobtubeUI* ui = (gx_boobtubeUI*)malloc(sizeof(gx_boobtubeUI));
...
static void cleanup(LV2UI_Handle handle) {
	gx_boobtubeUI* ui = (gx_boobtubeUI*)handle;
	cairo_destroy(ui->cr);
...
static void knob_expose(gx_boobtubeUI *ui,gx_controller* knob) {
...
static void draw_grid(gx_boobtubeUI *ui) {
	...
	if (ui->controls[0].adj.value > 0.9) {
		cairo_pattern_add_color_stop_rgba (pat, 1,  0.2*ui->controls[0].adj.value +
		  (0.05*ui->controls[4].adj.value), 0.0, 0.0, 1.0);
		cairo_pattern_add_color_stop_rgba (pat, 0.5,  0.15, 0.15,0.15+ (0.1*ui->controls[0].adj.value), 1.0);
		cairo_pattern_add_color_stop_rgba (pat, 0,  0.0, 0.0, 0.0 + (0.4*ui->controls[0].adj.value) +
		  (0.2*ui->controls[4].adj.value), 1.0);
	} else {
...
static void bypass_expose(gx_boobtubeUI *ui, gx_controller* switch_) {
	cairo_set_operator(ui->crf,CAIRO_OPERATOR_CLEAR);
	cairo_paint(ui->crf);
	cairo_set_operator(ui->crf,CAIRO_OPERATOR_OVER);
...
static void controller_expose(gx_boobtubeUI *ui, gx_controller * control) {
	cairo_push_group (ui->cr);
...
static void send_controller_event(gx_boobtubeUI *ui, int controller) {
...
static void check_is_active(gx_boobtubeUI *ui, int i, bool set) {
...
static bool aligned(int x, int y, gx_controller *control, gx_boobtubeUI *ui) {
...
static bool get_active_ctl_num(gx_boobtubeUI *ui, int *num) {
	static bool ret;
	ret = false;
	for (int i=0;i<CONTROLS;i++) {
...
static bool get_active_controller_num(gx_boobtubeUI *ui, int *num) {
	for (int i=0;i<CONTROLS;i++) {
...
static void scroll_event(gx_boobtubeUI *ui, int direction) {
	float value;
	int num;
...
static void enum_event(gx_boobtubeUI *ui, int i) {
	float value;
...
static void motion_event(gx_boobtubeUI *ui, double start_value, int m_y) {
...
static void set_key_value(gx_boobtubeUI *ui, int set_value) {
	float value = 0.0;
	int num;
	if (get_active_controller_num(ui, &num)) {
...
static void key_event(gx_boobtubeUI *ui, int direction) {
	float value;
	int num;
	if (get_active_controller_num(ui, &num)) {
		value = min(ui->controls[num].adj.max_value,max(ui->controls[num].adj.min_value, 
...
static void set_previous_controller_active(gx_boobtubeUI *ui) {
	int num;
	if (get_active_controller_num(ui, &num)) {
...
static void set_next_controller_active(gx_boobtubeUI *ui) {
	int num;
	if (get_active_controller_num(ui, &num)) {
		ui->controls[num].is_active = false;
		send_controller_event(ui, num);
...
static void get_last_active_controller(gx_boobtubeUI *ui, bool set) {
...
static void event_handler(gx_boobtubeUI *ui) {
	XEvent xev;
