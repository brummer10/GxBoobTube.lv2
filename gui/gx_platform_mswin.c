...
void gx_gui_create_window_and_surface(gx_boobtubeUI *ui) {
	...
	static TCHAR szClassName[] = TEXT("gx_boobtubeUIClass");
	WNDCLASS wndclass = {0};
...
void gx_gui_destroy_main_window(gx_boobtubeUI *ui) {
	...
	UnregisterClass(TEXT("gx_boobtubeUIClass"), NULL);
}
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	gx_boobtubeUI *ui = (gx_boobtubeUI *)GetWindowLongPtr(hwnd, GWLP_USERDATA);
